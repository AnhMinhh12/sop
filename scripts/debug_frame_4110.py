...
def main():
    ...
    cap.set(cv2.CAP_PROP_POS_FRAMES, 4110)
    ret, frame = cap.read()
    cap.release()
    
    if not ret:
        print("Failed to read frame 4110.")
        return
        
    cv2.imwrite("frame_4110.jpg", frame)
    
    sess = ort.InferenceSession(model_path, providers=["CPUExecutionProvider"])
    
    for mode in ["stretch", "letterbox"]:
        print(f"\n================ Frame 4110 Mode: {mode.upper()} ================")
        pred = run_inference(sess, frame, mode)
        hand_scores = pred[:, 4]
        robot_scores = pred[:, 5]
        sp_scores = pred[:, 6]
        ...
        for cls_idx, name in enumerate(class_names):
            ...
            if max_s > 0.05:
                # Get bounding box
                cx, cy, bw, bh = pred[top_idx, :4]
                print(f"  Detected {name} with conf {max_s:.4f} at anchor {top_idx}, bbox=[{cx:.1f}, {cy:.1f}, {bw:.1f}, {bh:.1f}]")
            else:
