...
class SpatialEngine:
    ...
    def __init__(self, sop_config: Dict[str, Any]):
        ...
        self.config = sop_config.get("config", {"w": 1280, "h": 720})
        ...
        # Trạng thái chi tiết từng tay
        self.hand_states = {
            "left": {"zone": None, "entry_time": time.time()},
            "right": {"zone": None, "entry_time": time.time()}
        }
        
        # Lịch sử di chuyển: Lưu danh sách (vùng_name, timestamp)
    ...
    def _check_step_logic(self, step: Dict) -> bool:
        logic = step.get("logic")
        now = time.time()
        start_t = self.step_start_time # Chỉ công nhận các sự kiện sau mốc này
        ...
        if logic == "zone_trigger":
            ...
            if mode == "any":
                return self._visited(target, "left", start_t) or self._visited(target, "right", start_t)
            elif mode == "both":
                return self._visited(target, "left", start_t) and self._visited(target, "right", start_t)
            else:
                return self._visited(target, mode, start_t)
            ...
            if mode == "any":
                for s in ["left", "right"]:
                    st = self.hand_states[s]
                    entry = max(st["entry_time"], start_t)
                    if st["zone"] == target and (now - entry) >= min_dur: return True
                return False
            elif mode == "both":
                for s in ["left", "right"]:
                    st = self.hand_states[s]
                    entry = max(st["entry_time"], start_t)
                    if st["zone"] != target or (now - entry) < min_dur: return False
                return True
            else:
                st = self.hand_states[mode]
                entry = max(st["entry_time"], start_t)
                return st["zone"] == target and (now - entry) >= min_dur
            ...
            # Check trình tự ghé thăm trước (nếu có)
            if seq:
                for s in ["left", "right"]:
                    hist = [z for z, t in self.hand_history[s] if t >= start_t]
                    if not self._match_seq(hist, seq): return False
            
            is_touching = 0 < self.hand_dist < max_dist
            if target:
                in_zone = self.hand_states["left"]["zone"] == target and \
                          self.hand_states["right"]["zone"] == target
                return is_touching and in_zone
            return is_touching

        elif logic == "dual_task" or logic == "dual_task_return":
            l_zone = step.get("left_zone")
            r_zone = step.get("right_zone")
            return self._visited(l_zone, "left", start_t) and self._visited(r_zone, "right", start_t)

        return False

    def _visited(self, zone: str, side: str, since_t: float) -> bool:
        """Kiểm tra xem tay side đã ghé qua zone kể từ since_t chưa."""
        for z, t in self.hand_history[side]:
            if z == zone and t >= since_t: return True
        return False
